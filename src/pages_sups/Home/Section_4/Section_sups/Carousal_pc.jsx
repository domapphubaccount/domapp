...
const MyCarousel = () => {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty(
      "--product-src",
      `url('${items.find((item) => item.index === activeIndex)?.src}')`
    );
    ...
    const handleScroll = () => {
      if (carouselRef.current) {
        ...
        isInView
          ...
          : document.documentElement.style.setProperty(`--view`, `-100px`);
        setinview(isInView);
      }
    };
        ...
        >
          <ul>
            ...
            {items.map((item, index) => (
              <li
                key={index}
                className={`${item.index === activeIndex && "active_card"}`}
              >
                {item.index == activeIndex ? (
                  ...
                  <button
                    onClick={(e) => {
                      handleCarouselNav(item.index);
                      sliderRef.slickGoTo(item.index - 1);
        ...
        <div className={`carousal_card fade-in`}>
          <div
            className={`slider-video-container ${
              inView && "slider-video-container-normal"
            }`}
                      ...
                      <div className="h-100 card_inside">
                        <Row className="h-100">
                          ...
                          <Col
                            ...
                            md={8}
                            lg={8}
                            className={`d-flex align-items-center justify-content-center rounded transformed_video video_col`}
                          >
                            <img
                              alt=""
                              loading="lazy"
